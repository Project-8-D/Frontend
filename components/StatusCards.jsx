...
export default function StatusCards({ coords }) {
    ...
    return (
        <div className="flex flex-row sm:flex-col grow gap-4">
            <div className="cards cursor-pointer" onClick={() => router.push("/sightings?resolved=no&limit=off")}>
                <h1 className="text-center">Open</h1>
                <p className="cards-number">{unresolved.length}</p>
            </div>
            <div className="cards cursor-pointer" onClick={() => router.push("/sightings?resolved=no&soundtype=gunshot&limit=off")}>
                <h1 className="text-center">Urgent</h1>
                <p className="cards-number">{urgent.length}</p>
            </div>
            <div className="cards cursor-pointer" onClick={() => router.push("/sightings?resolved=yes&limit=off")}>
